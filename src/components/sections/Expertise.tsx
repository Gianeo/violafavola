      ...
      <Grid>
        <div className="col-span-4 flex flex-col gap-6 pb-6 md:col-start-4 md:col-span-7 md:gap-8 md:pb-8">
          <div className="flex flex-col gap-6 md:flex-row md:gap-4">
            {items.map((item) => (
              <div key={item.title} className="md:flex-1">
                <div className="flex items-center gap-4">
                  <span className="flex size-12 items-center justify-center rounded-full bg-transparent">
                    <Icon src={item.icon} alt={item.title} size={20} />
                  </span>
                  <h3 className="copy-base font-medium text-neutral-lighter">
